...
import { Calendar, Ship, Anchor, Activity, Train, Settings2, ChevronDown, ChevronUp } from 'lucide-react';
...
export function ImportPlanner() {
  ...
  useEffect(() => {
    if (impRunResult) setShowForm(false);
  }, [impRunResult]);

  // Re-open form when reset (result cleared)
  useEffect(() => {
    if (!impRunResult) setShowForm(true);
  }, [impRunResult]);
